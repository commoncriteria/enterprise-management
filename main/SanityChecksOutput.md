* Error: f-element FIA_AFL.1.1  appears not to have an associated evaluation activity.:
        /Module[1]""/section[4]""/sel-sfrs[1]""/section[3]""/f-component[1]""/f-element[1]""
* Error: f-element FIA_AFL.1.2  appears not to have an associated evaluation activity.:
        /Module[1]""/section[4]""/sel-sfrs[1]""/section[3]""/f-component[1]""/f-element[2]""
* Error: f-element FIA_ENR_EXT.1.2  appears not to have an associated evaluation activity.:
        /Module[1]""/section[4]""/impl-dep-sfrs[1]""/section[5]""/f-component[1]""/f-element[2]""
* Warning: This document still has at least one comment.
* Error: Detected dangling id-reference to sel_all_ssh from attribute
        on-sel/Module[1]""/include-pkg[1]""/depends[1]""/on-sel[1]""
* Error: Detected multiple elements with an id of 'sec-sel-audit-table'.
* Error: Detected multiple elements with an id of 'sec-sel-audit-table'.
* Error: Detected multiple elements with an id of 'impl-dep-ftp'.
* Error: Detected multiple elements with an id of 'impl-dep-ftp'.
* Warning: Detected an empty _p_ element./Module[1]""/section[1]""/section[2]"The PP-Mo"/h:p[1]""
* Warning: Detected an empty _p_ element./Module[1]""/section[1]""/section[2]"The PP-Mo"/section[1]"The bound"/h:p[1]""
* Warning: Detected an empty _p_ element./Module[1]""/section[1]""/section[2]"The PP-Mo"/section[1]"The bound"/h:p[2]""
* Warning: Detected an empty _p_ element./Module[1]""/section[1]""/section[2]"The PP-Mo"/section[1]"The bound"/h:p[3]""
* Warning: Detected an empty _p_ element./Module[1]""/section[4]""/man-sfrs[1]""/section[4]""/f-component[2]""/f-element[1]""/note[1]"This requ"/h:p[1]""
* Warning: Detected an empty _p_ element./Module[1]""/section[4]""/sel-sfrs[1]""/section[6]""/f-component[1]""/f-element[3]""/note[1]"While the"/h:p[1]""
* Warning: Detected an empty _p_ element./Module[1]""/section[4]""/impl-dep-sfrs[1]""/section[4]""/f-component[1]""/f-element[1]""/note[1]"This requ"/h:p[1]""
* Warning: Since FCO_CPC_EXT.1.2 has an 'id' attribute in a descendant node in the title, you probably need to define an alternative 'ext-comp-def-title'.
                       
* Warning: Since FDP_MON_EXT.1.1 has an 'id' attribute in a descendant node in the title, you probably need to define an alternative 'ext-comp-def-title'.
                       
